...
The goal of these conventions is to make it easy and straightforward to issue SBTs on Algorand. These ASAs can represent commitments, credentials and affiliations and should be publicly visible and non-transferable (but possibly revocable-by-the-issuer). Throughout this ARC we illustrate a way to achieve this behaviour.

Most of the ideas exposed in here have been inspired by [Decentralized Society: Finding Web3's Soul](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4105763) and by [this discussion](https://ethereum-magicians.org/t/eip-5114-soulbound-badges/9417/57).

## Rationale

These conventions take inspiration from [EIP-5114](https://eips.ethereum.org/EIPS/eip-5114). 

The main differences are highlighted below:

* Asset Name, Unit Name, and URL are specified in the ASA parameters. This allows applications to efficienty display meaningful information, even if they aren't aware of ARC-5114 metadata.
* MIME types help clients more effectively fetch and render media.
* All asset metadata is stored onchain.
* Metadata can be mutable (for immutability refer to [ARC-69](https://github.com/algorandfoundation/ARCs/blob/main/ARCs/arc-0069.md)).
...
The main difference between ARC-5114 and ARC-69 is that the former calls for mutability, in order for the issuer to be able to freeze the SBT, issue a new version of it or revoke it. This is why the freeze account, the clawback account and the manager account **MUST** be provided and they all **SHOULD** be under the control of the ASA creator. There are no requirements regarding the reserve account of the ASA.

### JSON Metadata File Schema

ARC-5114 metadata is an extension of the ARC-69 metadata but with the 
...
```
The `standard` and the `status` fields are **REQUIRED** and **MUST** equal "ARC5114". All other fields are **OPTIONAL**. If provided, the other fields **MUST** match the description in the JSON schema.
